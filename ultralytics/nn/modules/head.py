...
from .conv import Conv, PyramidPooling, FFM, RFB2
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init

__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "SegmentCity"
...
class SegMaskPSP(nn.Module):  # PSP头，多了RFB2和FFM，同样砍了通道数，没找到合适的位置加辅助损失，因此放弃辅助损失
    def __init__(self, n_segcls=19, n=1, c_hid=256, ch=()):  # n是C3的, (接口保留了,没有使用)c_hid是隐藏层输出通道数（注意配置文件s*0.5,m*0.75,l*1）
        ...
        self.out = nn.Sequential(  # 实验表明引入较浅非线性不太强的层做分割会退化成检测的辅助(分割会相对低如72退到70,71，检测会明显升高)，PP前应加入非线性强一点的层并适当扩大感受野
                                ...
                                PyramidPooling(c_hid, k=[2, 4, 6, 12]),  # 按原文1,2,3,6，PSP加全局更好，但是ASPP加了全局后出现边界破碎
...
class SegmentCity(Detect):
    """YOLOv8 Segment head for segmentation cityscape models."""
    def __init__(self, nc=10, ns=19, c_hid=256, ch=()):
        """Initialize the YOLO model attributes such as the number of masks, prototypes, and the convolution layers."""
        super().__init__(nc, (ch[0], ch[1], ch[2]))
        self.detect = Detect.forward
        self.seg_head = SegMaskPSP(n_segcls=ns, n=1, c_hid=c_hid, ch=(ch[3], ch[4], ch[5]))  # 分割头接受ch[3], ch[4], ch[
        # 5] 分割头
    ...
    def forward(self, x):
        ...
        det_x = [x[0], x[1], x[2]]
        seg_x = [x[3], x[4], x[5]]
        out_det_x = self.detect(self, det_x)
        out_seg_x = self.seg_head(seg_x)
        if self.training:
            return out_det_x, out_seg_x
        return torch.cat([out_det_x, out_seg_x], 1) if self.export else (torch.cat([out_det_x[0], out_seg_x], 1), (out_det_x[1], out_seg_x))
